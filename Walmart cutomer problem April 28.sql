...
SELECT MONTH,DESCRIPTION,AMOUNTPAID,CUSTOMERID,RNK
FROM
(SELECT EXTRACT(MONTH FROM INVOICEDATE) AS MONTH,
 		CUSTOMERID,
        DESCRIPTION,
        SUM(UNITPRICE*QUANTITY)AS AMOUNTPAID,
	    RANK()OVER(PARTITION BY EXTRACT(MONTH FROM INVOICEDATE),CUSTOMERID ORDER BY SUM(UNITPRICE*QUANTITY) DESC)AS RNK
	    FROM WALMART_EU
GROUP BY MONTH,DESCRIPTION,CUSTOMERID) AS SUBQUERY
WHERE RNK=1
